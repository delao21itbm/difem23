...
import static com.gem.sistema.util.Constants.SEPARATOR_FILE_SYSTEM_DIR;
...
import static com.gem.sistema.util.UtilJPA.addQualifiersLikeFieldsString;
import static com.gem.sistema.util.UtilJPA.clearQualifiersLikeFieldsString;
...
/**
 * The Class CommonCatalogMB.
 *
 * @author Juan Carlos Pedraza Alcala
 * @param <T> the generic type
 */
public abstract class CommonCatalogMB<T extends AbstractEntity> extends AbstractMB {

	/** Constante para utilizar el log de la aplicacion. */
	private static final Logger LOGGER = LoggerFactory.getLogger(CommonCatalogMB.class);
	...
	/** The Constant NUMERO_FILAS_POR_TABLA. */
	private static final int NUMERO_FILAS_POR_TABLA = 10;

	/** The and id sector. */
	protected String AND_ID_SECTOR;

	/** The s sql csv. */
	protected String sSqlCsv;
	...
	/**
	 * Sets the util report.
	 *
	 * @param utilReport the new util report
	 */
	public void setUtilReport(UtilReport utilReport) {
	...
	protected boolean isValidSaveOrUpdate(final List<T> validateUnique, final T catalog) {
		LOGGER.info("validateUnique: " + validateUnique);
	...
	public StreamedContent getFile() {
		...
		if (CollectionUtils.isEmpty(getList())) {
			...
			result = utilReport.generateContentReportPlainText(getList(), headersReportTextPlain,
	...
	public StreamedContent getCsvFile() {
		...
		if (CollectionUtils.isEmpty(getList())) {
			...
			result = utilReport.generateContentReportCSV(getList(), headersReportCSV, TYPE_CONTENT_CSV,
	...
	public StreamedContent getFilePdf(final String pathReport, final String reportName) {
		...
		if (CollectionUtils.isEmpty(getList())) {
			...
			result = utilReport.generateContentReportPdf(getParametersReport(), REPORT_PATH_JASPER.concat(pathReport),
					reportName);
		}
		return result;
	}
	...
	public StreamedContent getFileXls(final String pathReport, final String reportName) {
		...
		if (CollectionUtils.isEmpty(getList())) {
			...
			result = utilReport.generateContentReporXls(getParametersReport(), getList(),
	...
	protected Map<String, Object> getParametersReport() {
		...
		// utilReport.putGeneralParametersReport(result);
		result.put("pHeader1", conctb.getNomDep());
		result.put(PARAMETER_WHERE_CLAUSE, utilReport.generateWhereClause(getBeanFind()));
		...
		if (this.getUserDetails().getIdSector() == 1) {
			result.put("pTitulo1", this.getUserDetails().getMunicipio());
		} else {
	...
	public void activateRowEdit(final int index) {
		...
		int realIdx = index % NUMERO_FILAS_POR_TABLA;
	...
	/**
	 * Gets the file csv by pl.
	 *
	 * @return the file csv by pl
	 */
	public String getFileCsvByPl() {
		return sSqlCsv + " " + utilReport.generateWhereClause(getBeanFind()) + AND_ID_SECTOR;
