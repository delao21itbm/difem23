 ...
 *
 */
@ManagedBean(name = "eAPESubProgramaMB")
@ViewScoped
public class EAPESubProgramaMB extends BaseDirectReport {
	...
	public void init() {
		...
		listMuninep = muninepRepository.getListSubPrograma(this.getUserDetails().getIdSector());
	...
	public String generateQuery(Integer idsector, Integer mes, String xdire) {
		...
		sSql.append(" FROM").append(" PASO P JOIN NATGAS N ").append(" ON N.CLVGAS= P.PARTIDA ")
				...
				.append(" AND SUBSTR(P.PROGRAMA,1,10)='" + xdire + "'").append(" GROUP BY P.PARTIDA,N.NOMGAS)")
