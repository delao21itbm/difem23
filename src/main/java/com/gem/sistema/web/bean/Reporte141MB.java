...
public class Reporte141MB extends BaseDirectReport {
	...
	private static final String TABLE_NAME = "PM3811";
	private static final String JASPER_NAME = "CompetenciaLaboral";
	private String COMPETENCIA_NAME = "Director de Obras";
	private String TITLE_LABEL = "Título profesional en ingeniería, arquitectura o área a fin";

	private List<TcPeriodo> listSemestres;
	private List<CompetenciaLaboralGenericDTO> listPm3811;
	private Integer semestre;
	private CompetenciaLaboralGenericDTO pm3811Selected;
	...
	public void init() {
		...
		listPm3811 = competenciasLaboralesService.findAll(TABLE_NAME);
		...
		if (CollectionUtils.isEmpty(listPm3811))
			bBorrar = true;
		bModificar = true;
		if (CollectionUtils.isNotEmpty(listPm3811)) {
			pm3811Selected = listPm3811.get(0);
			semestre = pm3811Selected.getSemestre();
			bModificar = false;
		}
		editando = false;
		currentPage = 0;
	...
	public void salvar() {
		CompetenciaLaboralGenericDTO nuevo = listPm3811.get(currentPage);
		nuevo.setIdanio(0);
		nuevo.setIdref(0);
		...
		if (isNew) {
			...
			if (editando && this.notNull(nuevo) && existe == 0) {
				...
				listPm3811 = this.competenciasLaboralesService.findAll(TABLE_NAME);
				bModificar = false;
				bCancelar = true;
				bBorrar = false;
				...
				generateNotificationFront(FacesMessage.SEVERITY_WARN, "Error!", "Ya existe un registro con el semestre " + nuevo.getSemestre());
			}
		} else if (!isNew) {
			this.notNull(nuevo);
			this.competenciasLaboralesService.update(TABLE_NAME, nuevo);
			this.listPm3811 = this.competenciasLaboralesService.findAll(TABLE_NAME);
			bModificar = false;
			editando = false;
			bCancelar = true;
			bBorrar = false;
	...
	public void modificar() {
		pm3811Selected = listPm3811.get(currentPage);
		listPm3811.set(currentPage, pm3811Selected);
	...
	public void reset() {
		listPm3811 = competenciasLaboralesService.findAll(TABLE_NAME);
		if (CollectionUtils.isEmpty(listPm3811)) {
			listPm3811.add(new CompetenciaLaboralGenericDTO());
		}
		editando = false;
		bBorrar = false;
		bCancelar = true;
	...
	public void clean() {
		if (listPm3811.get(currentPage).getIdrow() == 0) {

			listPm3811.get(currentPage).setSemestre(listSemestres.get(0).getPeriodo());
			listPm3811.get(currentPage).setExperiencia(StringUtils.EMPTY);
			listPm3811.get(currentPage).setFechaing(StringUtils.EMPTY);
			listPm3811.get(currentPage).setNombre(StringUtils.EMPTY);
			listPm3811.get(currentPage).setCargo(StringUtils.EMPTY);
			listPm3811.get(currentPage).setCertificacion(0);
			listPm3811.get(currentPage).setAntecedentes(0);
			listPm3811.get(currentPage).setConstancia(0);
			listPm3811.get(currentPage).setIssemym(0);
			listPm3811.get(currentPage).setTitulo(0);
	...
	public void adicionar() {
		...
		if (StringUtils.EMPTY == listPm3811.get(0).getFechaing()) {
			for (int i = 0; i < listPm3811.size(); i++) {
				listPm3811.remove(i);
			}
		}

		listPm3811.add(new CompetenciaLaboralGenericDTO(this.getUserDetails().getUsername(),
		...
		if (listPm3811.size() == 2) {
			RequestContext.getCurrentInstance()
					.execute("PF('dataGrid').paginator.setPage(" + (listPm3811.size() - 1) + ");");
	...
	public void borrar() {
		if (listPm3811.size() == 1) {
			currentPage = 0;
		}

		pm3811Selected = listPm3811.get(currentPage);

		this.competenciasLaboralesService.deleteRow(TABLE_NAME, pm3811Selected.getIdrow());
		listPm3811 = this.competenciasLaboralesService.findAll(TABLE_NAME);

		if (this.listPm3811.isEmpty()) {
			this.listPm3811.add(new CompetenciaLaboralGenericDTO());
			if (StringUtils.isEmpty(listPm3811.get(0).getFechaing()))
				bBorrar = true;
			bModificar = true;
		}
	...
	public Map<String, Object> getParametersReports() throws ReportValidationException {
		...
		firma = this.puestosFirmasService.getFirmaBySectorAnioClave(sector, 0L, ConstantsClaveEnnum.CLAVE_F06.getValue());
	...
	public void generatePDF() {
		...
		for (CompetenciaLaboralGenericDTO genericDTO : listPm3811) {
	...
	private void actualizaSeleccionado() {
		if (CollectionUtils.isNotEmpty(listPm3811)) {
			...
			pm3811Selected = listPm3811.get(listPm3811.size() - 1);
			semestre = pm3811Selected.getSemestre();
		}
	...
	public void addElement() {
		...
		salida: for (CompetenciaLaboralGenericDTO pm : listPm3811) {
			if (StringUtils.isEmpty(pm.getFechaing())) {
				listPm3811.remove(pm);
				break salida;
			}
		}
		if (CollectionUtils.isEmpty(listPm3811)) {

			listPm3811 = new ArrayList<CompetenciaLaboralGenericDTO>();
			listPm3811.add(0, dto);
		}
	}
	...
	public List<CompetenciaLaboralGenericDTO> getListPm3811() {
		return listPm3811;
	}

	public void setListPm3811(List<CompetenciaLaboralGenericDTO> listPm3811) {
	...
	public CompetenciaLaboralGenericDTO getPm3811Selected() {
		return pm3811Selected;
	}

	public void setPm3811Selected(CompetenciaLaboralGenericDTO pm3811Selected) {
		this.pm3811Selected = pm3811Selected;
	}
