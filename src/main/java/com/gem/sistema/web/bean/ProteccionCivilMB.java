...
@ViewScoped
public class ProteccionCivilMB extends BaseDirectReport {

	private static final String TABLE_NAME = "PROTECCION";
	private static final String JASPER_NAME = "CompetenciaLaboral";
	private String COMPETENCIA_NAME = "Coordinador Municipal de Protección Civil";
	private String TITLE_LABEL = "Título Profesional de Educación Superior";

	private List<TcPeriodo> listSemestres;
	private List<CompetenciaLaboralGenericDTO> listProteccion;
	private Integer semestre;
	private CompetenciaLaboralGenericDTO proteccionSelected;
	...
	public void init() {
		...
		listProteccion = competenciasLaboralesService.findAll(TABLE_NAME);
		...
		if (CollectionUtils.isNotEmpty(listProteccion)) {
			proteccionSelected = listProteccion.get(0);
			semestre = proteccionSelected.getSemestre();
			bModificar = false;
		}
		editando = false;
		currentPage = 0;
	...
	public void salvar() {
		CompetenciaLaboralGenericDTO nuevo = listProteccion.get(currentPage);
		...
		if (isNew) {
			...
			if (editando && this.notNull(nuevo) && existe == 0) {
				...
				listProteccion = this.competenciasLaboralesService.findAll(TABLE_NAME);
				bModificar = false;
				bCancelar = true;
				bBorrar = false;
			...
			this.listProteccion = this.competenciasLaboralesService.findAll(TABLE_NAME);
			bModificar = false;
			editando = false;
			bCancelar = true;
			bBorrar = false;
	...
	public void modificar() {
		proteccionSelected = listProteccion.get(currentPage);
		listProteccion.set(currentPage, proteccionSelected);
	...
	public void reset() {
		listProteccion = competenciasLaboralesService.findAll(TABLE_NAME);
		if (CollectionUtils.isEmpty(listProteccion)) {
			listProteccion.add(new CompetenciaLaboralGenericDTO());
		}
		editando = false;
		bBorrar = false;
		bCancelar = true;
	...
	public void clean() {
		if (listProteccion.get(currentPage).getIdrow() == 0) {

			listProteccion.get(currentPage).setSemestre(listSemestres.get(0).getPeriodo());
			listProteccion.get(currentPage).setExperiencia(StringUtils.EMPTY);
			listProteccion.get(currentPage).setFechaing(StringUtils.EMPTY);
			listProteccion.get(currentPage).setNombre(StringUtils.EMPTY);
			listProteccion.get(currentPage).setCargo(StringUtils.EMPTY);
			listProteccion.get(currentPage).setCertificacion(0);
			listProteccion.get(currentPage).setAntecedentes(0);
			listProteccion.get(currentPage).setConstancia(0);
			listProteccion.get(currentPage).setIssemym(0);
			listProteccion.get(currentPage).setTitulo(0);
			listProteccion.get(currentPage).setCursos(0);
	...
	public void adicionar() {
		...
		if (StringUtils.EMPTY == listProteccion.get(0).getFechaing()) {
			for (int i = 0; i < listProteccion.size(); i++) {
				listProteccion.remove(i);
			}
		}

		listProteccion.add(new CompetenciaLaboralGenericDTO(this.getUserDetails().getUsername(),
	...
	public void borrar() {
		...
		this.competenciasLaboralesService.deleteRow(TABLE_NAME, proteccionSelected.getIdrow());
		listProteccion = this.competenciasLaboralesService.findAll(TABLE_NAME);

		if (this.listProteccion.isEmpty()) {
			this.listProteccion.add(new CompetenciaLaboralGenericDTO());
	...
	public Map<String, Object> getParametersReports() throws ReportValidationException {
		...
		firma = this.puestosFirmasService.getFirmaBySectorAnioClave(sector, 0L, ConstantsClaveEnnum.CLAVE_F34.getValue());
	...
	public void generatePDF() {
		...
		for (CompetenciaLaboralGenericDTO genericDTO : listProteccion) {
	...
	private void actualizaSeleccionado() {
		if (CollectionUtils.isNotEmpty(listProteccion)) {
			...
			proteccionSelected = listProteccion.get(listProteccion.size() - 1);
			semestre = proteccionSelected.getSemestre();
		}
	...
	public void addElement() {
		...
		salida: for (CompetenciaLaboralGenericDTO pm : listProteccion) {
			if (StringUtils.isEmpty(pm.getFechaing())) {
				listProteccion.remove(pm);
				break salida;
			}
		}
		if (CollectionUtils.isEmpty(listProteccion)) {

			listProteccion = new ArrayList<CompetenciaLaboralGenericDTO>();
			listProteccion.add(0, dto);
		}
	}
	...
	public List<CompetenciaLaboralGenericDTO> getlistProteccion() {
		return listProteccion;
	}

	public void setlistProteccion(List<CompetenciaLaboralGenericDTO> listProteccion) {
	...
	public CompetenciaLaboralGenericDTO getproteccionSelected() {
		return proteccionSelected;
	}

	public void setproteccionSelected(CompetenciaLaboralGenericDTO proteccionSelected) {
		this.proteccionSelected = proteccionSelected;
	}
