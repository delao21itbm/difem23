...
/**
 * The Class PresupuestoIngresoIntegradoMB.
 */
@ManagedBean(name = "presupuestoIngresoIntegradoMB")
@SessionScoped
public class PresupuestoIngresoIntegradoMB extends AbstractMB {
	...
	public void genetareFile(){
		if(isNotNullCveRef(cveRef) == Boolean.TRUE) {
			IntegradoEgreso integrad = presupuestoIntegradoService.exeutePresupuestoIngreso(this.getUserDetails().getUsername(), mes, cveRef, this.getUserDetails().getIdMunicipio(), this.getUserDetails().getIdSector());
			if(integrad.getCodError() > 0) {
				sendFileToUser(integrad.getPathFile() + "/" + integrad.getFileName());
				FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Info", integrad.getMsgError() );
