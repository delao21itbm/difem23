...
import com.gem.sistema.business.domain.Pm2811;
import com.gem.sistema.business.domain.TrPuestoFirma;
import com.gem.sistema.business.repository.catalogs.ConctbRepository;
import com.gem.sistema.business.repository.catalogs.Pm2811Repository;
...
/**
 * The Class ReporteIndAnualTratamientoDesechosSolidosMB.
 */
@ManagedBean
@ViewScoped
public class ReporteIndAnualTratamientoDesechosSolidosMB extends BaseDirectReport {

	/** The master list. */
	private List<Pm2811> masterList = new ArrayList<Pm2811>();
	...
	/** The pm 2811 repository. */
	@ManagedProperty("#{pm2811Repository}")
	private Pm2811Repository pm2811Repository;
	...
	@PostConstruct
	public void init() {
		LOGGER.info(":: En postconsruct ReporteIndAnualTratamientoDesechosSolidosMB ");
		jasperReporteName = "reporte147";
	...
	public Map<String, Object> getParametersReports() throws ReportValidationException {
		...
		parameters.put("municipioClave", conctbRepository.findByIdsector(this.getUserDetails().getIdSector()).getClave());
	...
	/**
	 * metodo que se llama cuando se carga la pagina Pm2811.
	 */
	...
	/**
	 * metodo que busca todas las pm2811 en la base de datos y pone los valores
	 * iniciales.
	 */
	private void findAllPm2811() {
		masterList.clear();
		masterList = pm2811Repository.findAll();
		this.estadoInicialBotones();
		if (masterList.isEmpty()) {
			Pm2811 pm2811 = new Pm2811();
			masterList.add(pm2811);
		}
	...
	public void estadoInicialBotones() {
		if (!pm2811Repository.findAll().isEmpty()) {
	...
	/**
	 * Modifica un registro editado de Pm2811.
	 */
	public void modificar() {
		...
		if (masterList.isEmpty()) {
			Pm2811 pm2811 = new Pm2811();
			pm2811.setAnual(conctbRepository.findByIdsector(this.getUserDetails().getIdSector()).getAnoemp());
			masterList.add(pm2811);
		}
	...
	/**
	 * metodo que guarda una entidad en Pm2811.
	 */
	public void salvar() {
		try {
			...
			if (null == masterList.get(currentPage).getObsvdsg()){
				masterList.get(currentPage).setObsvdsg("");
			}
			if (null == masterList.get(currentPage).getObsvdst()){
				masterList.get(currentPage).setObsvdst("");
			}
			if (null == masterList.get(currentPage).getVdsg()){
				masterList.get(currentPage).setVdsg(BigDecimal.ZERO);
			}
			if (null == masterList.get(currentPage).getVdst()){
				masterList.get(currentPage).setVdst(BigDecimal.ZERO);
			}
			pm2811Repository.save(masterList.get(currentPage));
	...
	public void borrar() {
		...
		try {
			...
			try {
				pm2811Repository.delete(Long.valueOf(masterList.get(currentPage).getId().toString()).longValue());
				answer = true;
			} catch (Exception e) {
				answer = false;
			}
	 ...
	 */
	public List<Pm2811> getMasterList() {
		return masterList;
	}
	 ...
	 */
	public void setMasterList(List<Pm2811> masterList) {
	...
	/**
	 * Gets the pm 2811 repository.
	 *
	 * @return the pm 2811 repository
	 */
	public Pm2811Repository getPm2811Repository() {
		return pm2811Repository;
	}

	/**
	 * Sets the pm 2811 repository.
	 *
	 * @param pm2811Repository the new pm 2811 repository
	 */
	public void setPm2811Repository(Pm2811Repository pm2811Repository) {
		this.pm2811Repository = pm2811Repository;
	}
