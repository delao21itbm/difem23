...
import com.gem.sistema.business.service.catalogos.Pm2211Service;
import com.gem.sistema.business.service.reportador.ReportValidationException;

// TODO: Auto-generated Javadoc
/**
 * The Class AtencionIntegralAdolescentesSituacionVulnerableMB.
 */
@ManagedBean
@ViewScoped
public class AtencionIntegralAdolescentesSituacionVulnerableMB extends BaseDirectReport {
	...
	/** The service. */
	@ManagedProperty("#{pm2211Service}")
	private Pm2211Service service;

	/** The master list. */
	private List<Pm2211> masterList;

	/** The selected. */
	private Pm2211 selected = new Pm2211();
	...
	public void init() {
		...
		LOGGER.info(":: En postconsruct AtencionIntegralAdolescentesSituacionVulnerableMB - reporte 116");
		jasperReporteName = "reporte116";
	...
	public void inicializar() {
		...
		if (masterList == null || masterList.isEmpty()) {
			masterList = new ArrayList<Pm2211>();
			Pm2211 entity = new Pm2211();
	...
	public void adicionar() {
		Pm2211 last = masterList.isEmpty() ? null : masterList.get(masterList.size() - 1);
		...
		List<Integer> currentSemesters = masterList.stream().map(Pm2211::getTrimestre).collect(Collectors.toList());
	 ...
	 */
	public void salvar() {
		Pm2211 nuevo = noEmpty(masterList.get(currentPage));
		if (editando && validarNonNull(nuevo)) {
			...
			if (isNotLastMonth) {
				for (int i = masterList.indexOf(selected); i < masterList.size(); i++) {
					Pm2211 currentRecord = masterList.get(i);
					calculaAcumulados(currentRecord);
					Pm2211 entity = service.save(currentRecord);
				...
				Pm2211 pm2211 = service.save(nuevo);
				masterList.set(currentPage, pm2211);
			}
	 ...
	 */
	private boolean validarNonNull(Pm2211 nuevo) {
		boolean isValid = ((null == nuevo.getObsajb()) && (null == nuevo.getObstaj()));
		if (isValid) {
	...
	private void sortMasterList() {
		Collections.sort(masterList, new Comparator<Pm2211>() {
			@Override
			public int compare(Pm2211 h1, Pm2211 h2) {
	 ...
	 */
	private void defaultValues(Pm2211 currentRecord) {
		if (Objects.isNull(currentRecord.getAjb()))
			currentRecord.setAjb(0);
		if (Objects.isNull(currentRecord.getTaj()))
			currentRecord.setTaj(0);
	}
	...
	public void reset() {
		if (Objects.isNull(selected.getId())) {
			selected.setAjb(null);
			selected.setTaj(null);
			selected.setObsajb("");
			selected.setObstaj("");
		} else {
			selected = service.findById(selected.getId());
	 ...
	 * @param currentRecord the current record
	 */
	private void calculaAcumulados(Pm2211 currentRecord) {
		// Agarramos el registro anterior al registro seleccionado
		int lastIndex = masterList.indexOf(currentRecord) - 1;
		Pm2211 beforeSelected = lastIndex > -1 ? masterList.get(lastIndex) : null;
		// Si no existe anterior instanciamos un nuevo registro
		if (Objects.isNull(beforeSelected)) {
			if (Objects.isNull(beforeSelected)) {
				beforeSelected = new Pm2211();
				beforeSelected.setAcuajb(0);
				beforeSelected.setAcutaj(0);
			}
		}
		if (Objects.isNull(currentRecord.getAjb()))
			currentRecord.setAjb(0);
		if (Objects.isNull(currentRecord.getTaj()))
			currentRecord.setTaj(0);
		currentRecord.setAcuajb(beforeSelected.getAcuajb() + currentRecord.getAjb());
		currentRecord.setAcutaj(beforeSelected.getAcutaj() + currentRecord.getTaj());
	 ...
	 */
	public List<Pm2211> getMasterList() {
		return masterList;
	}
	 ...
	 */
	public void setMasterList(List<Pm2211> masterList) {
	 ...
	 * @return the service
	 */
	public Pm2211Service getService() {
		return service;
	}
	 ...
	 */
	public void setService(Pm2211Service service) {
	 ...
	 */
	public void setSelected(Pm2211 selected) {
	 ...
	 * @return the pm 2211
	 */
	public Pm2211 noEmpty(Pm2211 entity) {
		entity.setTaj(entity.getTaj() == null ? 0 : entity.getTaj());
		entity.setAjb(entity.getAjb() == null ? 0 : entity.getAjb());
		entity.setObstaj(entity.getObstaj() == null ? "" : entity.getObstaj());
		entity.setObsajb(entity.getObsajb() == null ? "" : entity.getObsajb());
		return entity;
	}
