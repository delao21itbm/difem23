...
@Repository
public class MediadorDAOImpl implements MediadorDAO {

	private static final Log LOGGER = LogFactory.getLog(MediadorDAOImpl.class);
	...
	public List<MediadorDTO> save(MediadorDTO MediadorDTO) {
		...
		try {
			map = GetValuesClassUtils.getFieldNamesAndValues(MediadorDTO, false);
	...
	public List<MediadorDTO> findByTableName(String tableName) {
		String sSql = "SELECT TV.ID_ROW,\n" + "       MAX(DECODE(TE.NOMBRE, 'SEMESTRE', TV.VALOR, NULL))SEMESTRE,\n"
				...
				+ "'  GROUP BY TV.ID_ROW";
		List<MediadorDTO> listValores = this.jdbcTemplate.query(sSql, new RowMapper<MediadorDTO>() {

			@Override
			public MediadorDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
				MediadorDTO mediadorDTO = new MediadorDTO();
				mediadorDTO.setIdRow(rs.getInt("ID_ROW"));
				mediadorDTO.setSemestre(rs.getInt("SEMESTRE"));
				mediadorDTO.setFechaIng(rs.getString("FECHAING"));
				mediadorDTO.setTitulo(rs.getString("TITULO"));
				mediadorDTO.setCertificacion(rs.getString("CERTIFICACION"));
				mediadorDTO.setEstudios(rs.getString("ESTUDIOS"));
				mediadorDTO.setCapturo(rs.getString("CAPTURO"));
				mediadorDTO.setIdSector(rs.getInt("IDSECTOR"));
	...
	public List<MediadorDTO> modify(MediadorDTO MediadorDTO) {
		tcValores = new TcValores();
		String sSql;
		Integer idRow = trEtqTablasRepository.getIdRow(MediadorDTO.getSemestre().toString(), TABLE_NAME);
		try {
			map = GetValuesClassUtils.getFieldNamesAndValues(MediadorDTO, false);
			for (Map.Entry<String, Object> entry : map.entrySet()) {
				System.out.println("Key : " + entry.getKey() + " Value : " + entry.getValue());
