...
import static com.gem.sistema.util.UtilJPA.isEmptyValueMethodGet;
...
public class UtilReport {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(UtilReport.class);
				...
				if (BooleanUtils.negate(columns == null) && columns.length > ZERO) {
					...
					if (BooleanUtils.negate(isEmptyValueMethodGet(resultMethodGet))) {
						...
						if (resultMethodGet instanceof String) {
							result.append(UtilJPA.FUNCTION_LOWER);
						}
						result.append(columns[ZERO].name());
						if (resultMethodGet instanceof String) {
							result.append(UtilJPA.CLOSING_PARENTHESIS);
						}
						if (resultMethodGet instanceof String) {
							result.append(LIKE);
							result.append(UtilJPA.FUNCTION_LOWER);
							result.append(SINGLE_QUOTE);
							if (!columns[ZERO].name().matches(UtilJPA.EXCLUDE_LEFT_REGEX)) {
							...
							if (entity instanceof Catflu && columns[ZERO].name().equalsIgnoreCase("clvflu")) {
								String clveflustr = null == ((Catflu) entity).getClvflu() ? StringUtils.EMPTY
										: ((Catflu) entity).getClvflu().toString();
								result.append(" like '");
								result.append(StringUtils.strip(clveflustr, "0") + "%");
								// result.append(resultMethodGet);
								result.append("' AND ");
								bandera = true;
							}
							if (entity instanceof Catben && columns[ZERO].name().equalsIgnoreCase("clvben")) {
								String clveflustr = null == ((Catben) entity).getClvben() ? StringUtils.EMPTY
										: ((Catben) entity).getClvben().toString();
								result.append(" >= ");
								result.append(clveflustr);
							} else {

								if ("com.gem.sistema.business.domain.Catprv".equals(entity.getClass().getName())
										&& "clvprv".equals(name)) {
									result.append(">");
								}
								if (bandera == false) {
